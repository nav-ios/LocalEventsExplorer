...
final class RemoteEventsLoaderTests: XCTestCase {
    ...
    func test_load_doesNotDeliverResultAfterSUTInstanceHasBeenDeallocated() {
        ...
        var sut: RemoteEventsLoader? = RemoteEventsLoader(url: url, client: client)

        var capturedResults = [RemoteEventsLoader.Result]()
        sut?.load { capturedResults.append($0) }

        sut = nil
        client.complete(withStatusCode: 200, data: makeEventsJSON([]))
    ...
    private func makeSUT(url: URL = URL(string: "https://any-url.com")!, file: StaticString = #filePath, line: UInt = #line) -> (sut: RemoteEventsLoader, client: HTTPClientSpy) {
        let client = HTTPClientSpy()
